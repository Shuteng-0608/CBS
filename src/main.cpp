...
std::vector<std::string> split(std::string s, char delim) {
	...
	std::vector<std::string> result; // store the position of obstacles into result
	std::stringstream ss(s);
	std::string item;
	
	while (getline(ss, item, delim)) {
		// A delimiter(delim) is one or more characters that separate text strings.
...
int main(int argc, char *argv[]) {
	...
	Map map = readMap("/home/shuteng/catkin_ws/src/ConflictBasedSearch/src/data/map3.txt");
